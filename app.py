...
# Load your models and scaler here
...
def preprocess_image(uploaded_file):
    ...
    img = Image.open(uploaded_file).convert('RGB')
    img = img.resize((image_height, image_width))  # Resize to match model input
    img_array = np.array(img) / 255.0  # Normalize the image
    return img_array

def predict_disease(img_array, model_choice):
    # Reshape and scale the input based on the model chosen
    img_flattened = img_array.flatten().reshape(1, -1)  # Flatten for KNN/SVM

    if model_choice == "svm_model.pkl":
        img_scaled = scaler.transform(img_flattened)  # Scale the features for SVM
        predictions = svm_model.predict(img_scaled)
    elif model_choice == "knn_model.pkl":
        predictions = knn_model.predict(img_flattened)
    else:
        predictions = None  # Handle case where model is not recognized
...
model_choice = st.selectbox("Choose a model", ("knn_model.pkl", "svm_model.pkl", "cnn_model.pkl"))

if uploaded_file is not None:
    img_array = preprocess_image(uploaded_file)  # Preprocess the image
    predictions = predict_disease(img_array, model_choice)  # Make predictions
