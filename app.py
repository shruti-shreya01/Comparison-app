...
# Set the IMAGE_SIZE based on your model's training
IMAGE_SIZE_KNN_SVM = 90  # 90x90 for a total of 8100 features for KNN/SVM
IMAGE_SIZE_CNN = 256  # 256x256 for CNN
...
def load_and_preprocess_image(image, model_type):
    if model_type == "CNN":
        ...
        img = load_img(image, target_size=(IMAGE_SIZE_KNN_SVM, IMAGE_SIZE_KNN_SVM))
    ...
    if model_type != "CNN":  # Flatten for KNN/SVM models
        img_array = img_array.flatten().reshape(1, -1)  # Reshape to 1 sample with flattened image
        if model_type == "SVM":
            img_array = img_array[:, :70]  # Ensure correct number of features for SVM
